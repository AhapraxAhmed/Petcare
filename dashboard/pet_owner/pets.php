...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($action === 'add_pet') {
        ...
        if (!$error) {
            $stmt = $conn->prepare("INSERT INTO pets (owner_id, name, species, breed, age, gender, weight, color, microchip_id, image, medical_history) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");
...
$shelters = $conn->query("SELECT id as shelter_id, name as shelter_name FROM users WHERE role = 'shelter'")->fetch_all(MYSQLI_ASSOC);
