...
$user = getCurrentUser();
$user_id = $user['id']; // users table uses 'id' not 'user_id' based on furshield.sql
...
$stmt->execute();
$total_pets = $stmt->get_result()->fetch_assoc()['count'];
...
$stmt->bind_param("i", $user_id);
$stmt->execute();
$upcoming_appointments = $stmt->get_result()->fetch_assoc()['count'];
...
$stmt->bind_param("i", $user_id);
$stmt->execute();
$recent_health_records = $stmt->get_result()->fetch_assoc()['count'];
...
$stmt->bind_param("i", $user_id);
$stmt->execute();
$recent_pets = $stmt->get_result()->fetch_all(MYSQLI_ASSOC);

// Get upcoming appointments details
$stmt = $conn->prepare("
    SELECT a.id as appointment_id, a.appointment_date, a.notes as reason, a.status,
...
$stmt->bind_param("i", $user_id);
$stmt->execute();
$upcoming_appointments_details = $stmt->get_result()->fetch_all(MYSQLI_ASSOC);

// Get recent notifications
$stmt = $conn->prepare("
    SELECT id as notification_id, title, message, type, created_at, is_read
    FROM notifications 
    WHERE user_id = ? 
    ORDER BY created_at DESC 
    LIMIT 5
